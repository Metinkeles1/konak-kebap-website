...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    h2: ({ children }) => (
      <h2 className="text-3xl md:text-4xl font-display text-cream mt-10 mb-4 leading-tight">
        {children}
      </h2>
    ),
    h3: ({ children }) => (
      <h3 className="text-2xl md:text-3xl font-display text-cream mt-8 mb-3">{children}</h3>
    ),
    p: ({ children }) => (
    ...
    blockquote: ({ children }) => (
      <blockquote className="border-l-4 border-gold pl-6 py-2 my-6 italic text-cream/80 bg-surface/50">
        {children}
      </blockquote>
    ),
